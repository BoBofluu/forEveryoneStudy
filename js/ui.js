...
function switchPage(pageName) {
    ...
    if (pageName === 'input') {
        ...
        titleEl.innerText = t('tab_list');
        renderList();
        renderCalendar();
    } else if (pageName === 'settings') {
        document.getElementById('pageSettings').classList.add('active');
        document.getElementById('tabSettings').classList.add('active');
        titleEl.innerText = t('tab_settings');
        
        // 更新當前語言顯示文字
        const langNames = { 'zh-TW': '繁體中文', 'ja': '日本語', 'ko': '한국어' };
        if (document.getElementById('currentLangLabel')) {
            document.getElementById('currentLangLabel').innerText = langNames[currentLang] || currentLang;
        }

        // 載入已儲存的 Gemini API Key
        ...
        <div class="filter-modal-content" style="text-align:left; max-height:500px; overflow-y:auto; padding:15px; background:var(--bg-color);">
            <div style="margin-bottom:20px;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
                    <h3 style="margin:0; font-size:18px; color:var(--text-main);">${t('btn_filter')}</h3>
                    <div style="display:flex; gap:10px;">
                        <button onclick="toggleAllFilters(true)" style="background:var(--color-grammar); border:none; color:#121212; padding:6px 12px; border-radius:6px; font-size:12px; cursor:pointer; font-weight:bold;">${t('all')}</button>
                        <button onclick="toggleAllFilters(false)" style="background:var(--color-danger); border:none; color:white; padding:6px 12px; border-radius:6px; font-size:12px; cursor:pointer; font-weight:bold;">${t('clear')}</button>
                    </div>
                </div>
                    ...
                    <div style="margin-bottom:25px; background:rgba(255,255,255,0.03); padding:15px; border-radius:12px; border:1px solid #333;">
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                            ...
                            <div style="display:flex; gap:8px;">
                                <button onclick="toggleSubcatInCat('${key}', true)" style="font-size:11px; background:#555; color:white; border:none; padding:4px 10px; border-radius:4px; cursor:pointer;">${t('btn_subcat_select_all')}</button>
                                <button onclick="toggleSubcatInCat('${key}', false)" style="font-size:11px; background:#555; color:white; border:none; padding:4px 10px; border-radius:4px; cursor:pointer;">${t('btn_subcat_clear')}</button>
                            </div>
                        </div>
                        <div style="display:flex; flex-wrap:wrap; gap:10px; padding-left:5px;">
                            ...
                            ${(value.subcats || []).length === 0 ? `<span style="font-size:12px; color:#666; font-style:italic;">${t('msg_no_subcat')}</span>` : ''}
                        </div>
                    </div>
                    `;
                }).join('')}
...
function renderList() {
    ...
    let filteredData = jpData.filter(item => {
        if (selectedFilterCats.size > 0 && !selectedFilterCats.has(item.cat)) return false;
        ...
        if (selectedDates.size > 0 && !selectedDates.has(dateStr)) return false;
        ...
        container.innerHTML = `
            <div style="text-align:center; color:var(--text-sub); margin-top:60px; display:flex; flex-direction:column; align-items:center; opacity:0.6;">
                <div style="font-size:16px; font-weight:500; color:var(--text-main); margin-bottom:5px;">
                    ${jpData.length === 0 ? t('msg_no_content') : t('msg_no_results')}
                </div>
            </div>`;
        return;
    }
    ...
    filteredData.forEach(item => {
        const createdDate = item.createdAt ? new Date(item.createdAt) : new Date(item.id);
        const itemMonthStr = createdDate.toLocaleString(currentLang, { year: 'numeric', month: '2-digit' });
        ...
        const catInfo = categoryMap[item.cat] || { label: t('msg_unknown_cat'), class: 'cat-other' };
        let subcatBadge = '';
        ...
        const dateStr = createdDate.toLocaleString(currentLang, { hour12: false, year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' });
        ...
        card.innerHTML = `
            <div class="item-header">
                ...
                <div style="display:flex; align-items:center; gap:10px;">
                    ...
                    <button class="delete-btn" onclick="deleteItem(${item.id}, event)" title="${t('btn_delete')}">✖</button>
                </div>
            </div>
            <div class="jp-text line-clamp-3">${displayTitle}${item.jp || item.en || ''}</div>
        `;
        if (currentGroupContent) currentGroupContent.appendChild(card);
...
window.initFuriganaEngine = function() {
    ...
    if (typeof kuromoji === 'undefined') { isInitializingTokenizer = false; return; }
    kuromoji.builder({ dicPath: 'dict/' }).build((err, _tokenizer) => {
        if (err) isInitializingTokenizer = false;
        else { tokenizer = _tokenizer; isInitializingTokenizer = false; }
    });
};

window.toggleFurigana = function (itemId) {
    ...
    if (isHidden) {
        ...
        if (!text) {
            displayBox.innerHTML = `<span style="color:#888;">${t('msg_no_content')}</span>`;
            switchToDisplay();
            return;
        }
        if (!tokenizer) {
            if (isInitializingTokenizer) Swal.fire({ icon: 'info', title: t('msg_engine_loading'), timer: 2000, showConfirmButton: false });
            else { initFuriganaEngine(); Swal.fire({ icon: 'warning', title: t('msg_engine_not_started'), text: t('msg_engine_restarting') }); }
            return;
        }
        btn.innerText = '...';
        btn.disabled = true;
        try {
            ...
            switchToDisplay();
        } catch (err) { btn.innerText = t('btn_show_furigana'); btn.disabled = false; }

        function switchToDisplay() {
            textarea.style.display = 'none';
            displayBox.style.display = 'block';
            btn.innerText = t('btn_edit_original');
        ...
        btn.innerText = t('btn_show_furigana');
...
function processText(rawText) {
    ...
    tokens.forEach(token => {
        ...
        if (hasKanji && token.reading && token.reading !== '*') {
            ...
            if (reading !== token.surface_form) html += `<ruby><rb>${token.surface_form}</rb><rt>${reading}</rt></ruby>`;
            else html += token.surface_form;
        } else html += token.surface_form;
    });
    return html.replace(/\n/g, '<br>');
}
...
function openDetail(id) {
    ...
    document.getElementById('detailContainer').innerHTML = `
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; flex-wrap:wrap; gap:10px;">
            <div style="display:flex; gap:8px;">
                <button class="action-btn" onclick="duplicateItem(${item.id})">${t('btn_duplicate')}</button>
                <button class="action-btn" onclick="exportItem(${item.id})" style="background-color:var(--color-grammar); color:#121212;">${t('btn_export_item')}</button>
                <button id="btn_toggleEn_${item.id}" class="action-btn" onclick="toggleEnInput(${item.id})" style="background-color:var(--input-bg); border:1px solid #555;">${item.en ? t('btn_hide_en') : t('btn_show_en')}</button>
            </div>
            <span style="font-size:12px; color:var(--text-sub);">${t('created_at')}: ${dateStr}</span>
        </div>

        <div class="input-group" style="display: flex; gap: 30px; margin-bottom: 20px; flex-wrap: wrap;">
            <div style="flex: 1; min-width: 300px;">
                <label style="display: block; margin-bottom: 10px;">${t('label_category')}</label>
                <div style="display: flex; gap: 10px; align-items: center;">
                    <select id="detailCat_${item.id}" onchange="confirmChangeCat(${item.id}, this)" style="flex: 1; height: 38px; margin-bottom: 0; padding: 6px 12px;">
                        ${Object.entries(categoryMap).map(([key, value]) => `<option value="${key}" ${item.cat === key ? 'selected' : ''}>${value.label}</option>`).join('')}
                    </select>
                    <button class="action-btn" onclick="manageCategories()" style="font-size: 13px; height: 38px; padding: 0 15px; background-color: var(--input-bg); border: 1px solid #555; white-space: nowrap;">${t('btn_manage_category')}</button>
                </div>
            </div>
            <div style="flex: 1; min-width: 300px;">
                <label style="display: block; margin-bottom: 10px;">${t('label_subcategory')}</label>
        ...
        <div class="input-group">
            ...
            <input type="text" value="${item.title || ''}" oninput="autoSave(${item.id}, 'title', this.value)" placeholder="${t('placeholder_title')}">
        </div>

        <div id="enInputGroup_${item.id}" style="display: ${item.en ? 'block' : 'none'}; margin-bottom: 20px;">
            <label>${t('label_en_content')}</label>
            <div class="jp-action-bar" style="margin-top: 8px;">
                <button class="action-btn" onclick="copyToClipboard('detailEn_${item.id}')" style="background-color:var(--input-bg);">${t('btn_copy')}</button>
                    <button id="btn_detailEnUS_${item.id}" class="action-btn" onclick="playAudio('detailEn_${item.id}', 'en-US', 'btn_detailEnUS_${item.id}')" title="${t('btn_tts_en_us')}">🇺🇸 ${t('btn_tts_en_us')}</button>
                    <button id="btn_detailEnGB_${item.id}" class="action-btn" onclick="playAudio('detailEn_${item.id}', 'en-GB', 'btn_detailEnGB_${item.id}')" title="${t('btn_tts_en_gb')}">🇬🇧 ${t('btn_tts_en_gb')}</button>
                    <button id="btn_detailEnAU_${item.id}" class="action-btn" onclick="playAudio('detailEn_${item.id}', 'en-AU', 'btn_detailEnAU_${item.id}')" title="${t('btn_tts_en_au')}">🇦🇺 ${t('btn_tts_en_au')}</button>
                </div>
                <textarea id="detailEn_${item.id}" oninput="autoResize(this); autoSave(${item.id}, 'en', this.value)" placeholder="${t('placeholder_content')}">${item.en || ''}</textarea>
            </div>
        </div>
        ...
        <div class="input-group">
            ...
            <div class="jp-action-bar">
                <button class="action-btn" onclick="copyToClipboard('detailJp_${item.id}')" style="background-color:var(--input-bg);">${t('btn_copy')}</button>
                <button id="btn_toggleFurigana_${item.id}" class="action-btn" onclick="toggleFurigana(${item.id})" style="background-color:var(--input-bg);">${t('btn_show_furigana')}</button>
                <select class="template-select" id="detailTemplateSelect" style="padding:4px 8px;"></select>
                <button class="action-btn" onclick="insertTemplate('detailJp_${item.id}', 'detailTemplateSelect', ${item.id})">${t('btn_insert_template')}</button>
                <button class="action-btn" onclick="manageTemplates()">${t('btn_manage_template')}</button>
                ${item.cat === 'vocab' || item.jp ? `
                <select id="ttsLangJp_${item.id}" style="padding:4px 8px; background:var(--input-bg); color:var(--text-main); border:1px solid #555; border-radius:6px;">
                    <option value="ja-JP">ja-JP / Despina</option>
                    <option value="en-US">en-US / Achernar</option>
                    <option value="en-AU">en-AU / Aoede</option>
                    <option value="en-GB">en-GB / Charon</option>
                </select>
                <button id="btn_detailJp_${item.id}" class="action-btn" onclick="playAudio('detailJp_${item.id}', document.getElementById('ttsLangJp_${item.id}').value, 'btn_detailJp_${item.id}')" title="${t('btn_tts_jp')}">🔊 ${t('btn_tts_jp')}</button>
                ` : ''}
            </div>
            <div id="furiganaBox_${item.id}" class="furigana-display-box"></div>
            <textarea id="detailJp_${item.id}" oninput="autoResize(this); autoSave(${item.id}, 'jp', this.value)" placeholder="${t('placeholder_content')}">${item.jp || ''}</textarea>
        </div>

        <div class="input-group">
            <label>${t('label_note')}</label>
            <textarea id="detailNote_${item.id}" oninput="autoResize(this); autoSave(${item.id}, 'note', this.value)" placeholder="${t('placeholder_note')}">${item.note || ''}</textarea>
        </div>
        
        <button onclick="deleteItem(${item.id})" style="margin-top:10px; width:100%; border:none; background-color:rgba(255,107,107,0.1); color:var(--color-danger); padding:15px; border-radius:10px; cursor:pointer; font-size:16px; font-weight:bold; transition:background-color 0.2s, transform 0.1s ease;">${t('btn_delete_item')}</button>
    `;
    ...
    document.getElementById('pageTitle').innerText = t('page_title_detail');
...
window.confirmChangeCat = function(itemId, selectEl) {
    const item = jpData.find(i => i.id === itemId);
    const newCat = selectEl.value;
    const oldCat = item ? item.cat : null;

    const hasSubcats = item && item.subcats && item.subcats.length > 0;

    if (hasSubcats && newCat !== oldCat) {
        const prevCat = oldCat;
        Swal.fire({
            title: t('btn_confirm'),
            text: t('msg_change_cat_confirm'),
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                autoSave(itemId, 'cat', newCat);
                autoSave(itemId, 'subcats', []);
                renderSubcategoryOptions(`detailCat_${itemId}`, `detailSubcatPills_${itemId}`, itemId);
            } else {
                selectEl.value = prevCat;
            }
        });
    } else {
        autoSave(itemId, 'cat', newCat);
...
window.saveGeminiApiKey = function() {
    ...
    if (key) {
        ...
        Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: t('msg_key_saved'), showConfirmButton: false, timer: 1500 });
    } else {
        localStorage.removeItem('geminiApiKey');
        Swal.fire({ toast: true, position: 'top-end', icon: 'info', title: t('msg_key_cleared'), showConfirmButton: false, timer: 1500 });
    }
};
...
window.renderCalendar = function() {
    ...
    const dayNames = t('day_names') || ["日", "一", "二", "三", "四", "五", "六"];
    ...
    `;
    for (let i = 0; i < startingDay; i++) html += `<div class="calendar-cell empty"></div>`;
    for (let i = 1; i <= totalDays; i++) {
        ...
        html += `<div class="calendar-cell ${isActive}" onclick="selectCalendarDate('${dateStr}')">${i}${hasNote}</div>`;
    }
    html += `</div>`;
